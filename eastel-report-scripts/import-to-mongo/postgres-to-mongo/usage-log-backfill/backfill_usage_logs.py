...
SYNC_FOLDER = Path(__file__).resolve().parent.parent / "postgres-to-mongo-usage-sync"
if str(SYNC_FOLDER) not in sys.path:
    sys.path.insert(0, str(SYNC_FOLDER))

from postgres_to_mongo_usage_sync import (  # noqa: E402
    DEFAULT_BATCH_SIZE,
...
DEFAULT_SOURCE_TABLE = "iot_portal_tb_usage_log"
DEFAULT_SYNCED_COLLECTION = "usage_synced"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill or repair usage log documents in MongoDB from PostgreSQL."
    )
    ...
    parser.add_argument("--ids", default="", help="Comma-separated explicit usage_log_id values to repair.")
    parser.add_argument(
...
def get_usage_synced_collection_name(config: dict[str, Any], default: str) -> str:
    mongo_config = config.get("mongo", {})
    if isinstance(mongo_config, dict):
        value = mongo_config.get("eastel_usage_synced_collection")
...
def ensure_usage_synced_collection_indexes(synced_collection) -> None:
    synced_collection.create_index(
        [("source_table", ASCENDING), ("mongo_collection", ASCENDING), ("usage_log_id", ASCENDING)],
        name="uq_usage_synced_source_collection_id",
        unique=True,
    )
    synced_collection.create_index(
        [("usage_log_id", ASCENDING)],
        name="ix_usage_synced_usage_log_id",
    )


def get_already_synced_usage_ids(
    synced_collection,
    source_table: str,
    ...
    if not usage_log_ids:
        return set()
    synced_doc_ids = [
        get_synced_doc_key(source_table, mongo_collection, usage_log_id)
        for usage_log_id in usage_log_ids
    ]
    cursor = synced_collection.find({"_id": {"$in": synced_doc_ids}}, {"usage_log_id": 1})
    return {
        int(doc["usage_log_id"])
        for doc in cursor
        if doc.get("usage_log_id") is not None
    }


def mark_usage_ids_synced(
    synced_collection,
    source_table: str,
    mongo_collection: str,
    usage_log_ids: list[int],
    ...
    operations = [
        UpdateOne(
            {"_id": get_synced_doc_key(source_table, mongo_collection, usage_log_id)},
            {
                "$set": {
...
def fetch_rows_by_time(
    ...
    with pg_conn.cursor() as cursor:
        cursor.execute(
            query,
            (start_dt.replace(tzinfo=None), end_dt.replace(tzinfo=None), cutoff, last_usage_log_id, batch_size),
        )
...
def fetch_rows_by_id_range(
    ...
    with pg_conn.cursor() as cursor:
        cursor.execute(query, (id_from, id_to, last_usage_log_id, batch_size))
        return cursor.fetchall()
...
def upsert_rows(mongo_collection, rows: list[dict[str, Any]], source_table: str) -> list[int]:
    ...
    return [int(row["usage_log_id"]) for row in rows]


def filter_unsynced_usage_rows(
    synced_collection,
    source_table: str,
    mongo_collection_name: str,
    rows: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], int]:
    candidate_ids = [int(row["usage_log_id"]) for row in rows]
    already_synced_ids = get_already_synced_usage_ids(
        synced_collection=synced_collection,
        source_table=source_table,
        mongo_collection=mongo_collection_name,
        usage_log_ids=candidate_ids,
    )
    filtered_rows = [
        row for row in rows
        if int(row["usage_log_id"]) not in already_synced_ids
    ]
...
def backfill_by_time(
    ...
    while True:
        ...
        rows_to_process, skipped_count = filter_unsynced_usage_rows(
            synced_collection=synced_collection,
            source_table=source_table,
            mongo_collection_name=mongo_collection_name,
            rows=rows,
        )
        synced_usage_ids = upsert_rows(mongo_collection, rows_to_process, source_table)
        mark_usage_ids_synced(synced_collection, source_table, mongo_collection_name, synced_usage_ids)
        total_processed += len(synced_usage_ids)
        percent = (total_processed / total_rows * 100.0) if total_rows > 0 else 100.0
        print(
            f"[{datetime.now().isoformat(timespec='seconds')}] "
            f"repaired {len(synced_usage_ids)} rows, skipped_already_marked={skipped_count}, total={total_processed}/{total_rows}, percent={percent:.2f}%, last_usage_log_id={last_usage_log_id}, "
...
def main() -> None:
    ...
    mongo_db_name = get_usage_mongo_db_name(config, str(get_config_value(config, "mongo", "database", "MONGO_DB", args.mongo_db)))
    ...
    synced_collection_name = get_usage_synced_collection_name(
        config,
    ...
    with psycopg.connect(postgres_dsn, row_factory=dict_row) as pg_conn:
        with get_mongo_client(config) as mongo_client:
            ...
            ensure_usage_synced_collection_indexes(synced_collection)

            if args.mode == "ids":
                print(
                    f"[{datetime.now().isoformat(timespec='seconds')}] "
                    "calculating potential affected rows before processing explicit usage_log_id list"
                )
                ...
                print(
                    f"[{datetime.now().isoformat(timespec='seconds')}] "
                    f"repaired {len(synced_usage_ids)} explicit usage_log_id rows, skipped_already_marked={skipped_count}"
                )
                return
            ...
            print(
                f"[{datetime.now().isoformat(timespec='seconds')}] "
                "calculating potential affected rows before processing "
                f"usage_log_id range id_from={id_from} id_to={id_to}"
            )
