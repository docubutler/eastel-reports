...
def fetch_rows_by_time(
    ...
    with pg_conn.cursor() as cursor:
        cursor.execute(query, (start_dt.replace(tzinfo=None), end_dt.replace(tzinfo=None), last_usage_log_id, batch_size))
...
def backfill_by_time(pg_conn, mongo_collection, source_table: str, time_field: str, start_dt: datetime, end_dt: datetime, batch_size: int, total_rows: int) -> int:
    ...
    while True:
        ...
        rows = fetch_rows_by_time(pg_conn, source_table, time_field, start_dt, end_dt, last_usage_log_id, batch_size)
        if not rows:
            break
        ...
        print(
            ...
            f"repaired {len(rows)} rows, total={total_processed}/{total_rows}, percent={percent:.2f}%, last_usage_log_id={last_usage_log_id}, "
    ...
    with psycopg.connect(postgres_dsn, row_factory=dict_row) as pg_conn:
        with get_mongo_client(config) as mongo_client:
            ...
            if args.mode == "ids":
                ...
                upsert_rows(mongo_collection, rows, source_table)
                print(f"[{datetime.now().isoformat(timespec='seconds')}] repaired {len(rows)} explicit usage_log_id rows")
                return
