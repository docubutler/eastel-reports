...
def inspect_rows(
    ...
    log(f"building expected usage documents for {len(rows)} PostgreSQL rows")
    expected_by_id = {
    ...
    log(f"fetching {len(expected_by_id)} usage documents from Mongo")
    mongo_docs = {
        int_id(doc["usage_log_id"]): doc
        for doc in mongo_collection.find({"usage_log_id": {"$in": list(expected_by_id)}}, projection)
        if doc.get("usage_log_id") is not None
    }
    log(
        f"fetched {len(mongo_docs)} usage documents from Mongo in "
    ...
    for usage_log_id, expected_doc in expected_by_id.items():
        ...
        if is_mismatched and len(batch_sample_ids) < min(sample_limit, 10):
            batch_sample_ids.append(usage_log_id)
        ...
        if print_detail_diffs and is_mismatched and mongo_doc is not None:
            ...
            log(f"detailed diffs for usage_log_id={usage_log_id}, diff_count_shown={len(diffs)}")
            ...
            log(f"detailed diffs for usage_log_id={usage_log_id}: Mongo document is missing")

    log(
        f"compared {len(rows)} usage rows in {time.perf_counter() - inspect_started_at:.3f}s, "
        f"batch_missing={missing_count}, batch_field_mismatches={field_mismatch_count}, "
        f"batch_sample_mismatched_usage_log_ids={batch_sample_ids}, "
...
def print_summary(
    ...
    print(f"sampled_mismatched_usage_log_ids={sample_ids}", flush=True)
...
def inspect_by_time(
    ...
    while True:
        ...
        log(f"fetching PostgreSQL usage batch after usage_log_id={last_usage_log_id}")
        ...
        log(f"fetched {len(rows)} PostgreSQL usage rows, last_usage_log_id={last_usage_log_id}")
...
def main() -> None:
    ...
    with psycopg.connect(postgres_dsn, row_factory=dict_row) as pg_conn:
        with get_mongo_client(config) as mongo_client:
            ...
            if args.mode == "ids":
                log("calculating total rows for explicit usage_log_id list")
                total_rows = count_rows_by_ids(pg_conn, source_table, explicit_ids)
                log(f"fetching {len(explicit_ids)} explicit PostgreSQL usage IDs")
                rows = fetch_rows_by_ids(pg_conn, source_table, explicit_ids)
                log(f"fetched {len(rows)} PostgreSQL usage rows for explicit ID list")
                sample_ids: list[int] = []
