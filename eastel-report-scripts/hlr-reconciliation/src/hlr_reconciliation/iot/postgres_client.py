...
from hlr_reconciliation.models.config import PostgresConfig
from hlr_reconciliation.models.records import SubscriberKey


class IotPostgresClient:
    ...
    def fetch_active_subscribers(self) -> list[SubscriberKey]:
        try:
            ...
            raise SourceDatabaseError("Failed to fetch IOT BSS subscribers from PostgreSQL") from exc
...
def _rows_to_keys(rows: Iterable[tuple[object, ...]]) -> list[SubscriberKey]:
    ...
    for row in rows:
        if len(row) < 2:
            raise SourceDatabaseError("IOT query must return msisdn as column 1 and imsi as column 2")
