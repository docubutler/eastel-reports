...
PREFERRED_SECTION_PATTERN = re.compile(r"(?m)^--\s*QUERY:\s*(\d+)\s*\|\s*(.+?)\s*$")
LEGACY_SECTION_PATTERN = re.compile(r"/\*\s*=+\s*(\d+)\.\s*(.*?)\s*=+\s*\*/", re.DOTALL)
START_DATE_PATTERN = re.compile(r"(t\.req_time\s*>=\s*)'[^']+'", re.IGNORECASE)
END_DATE_PATTERN = re.compile(r"(t\.req_time\s*<\s*)'[^']+'", re.IGNORECASE)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Split queries.sql into numbered SQL files with date variables.")
...
def extract_numbered_queries(source_text: str) -> list[tuple[int, str, str]]:
    ...
    if not numbered_queries:
        raise ValueError("No numbered queries found. Prefer '-- QUERY: <number> | <title>' markers.")
