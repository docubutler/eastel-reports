...
def resolve_query_path(base_dir: Path, queries_dir: Path, raw_path: str) -> Path:
    path = Path(raw_path)
    if path.is_absolute():
        return path
    if path.parent == Path("."):
        return (queries_dir / path).resolve()
...
def load_config(config_path: str | Path) -> AppConfig:
    ...
    postgres_raw = _require_mapping(data, "postgres")
    postgres = PostgresConfig(
        dsn=str(postgres_raw.get("dsn") or "").strip(),
        host=str(postgres_raw.get("host") or "localhost").strip(),
        port=str(postgres_raw.get("port") or "5432").strip(),
        database=str(postgres_raw.get("database") or "").strip(),
        user=str(postgres_raw.get("user") or "").strip(),
        password=str(postgres_raw.get("password") or "").strip(),
        sslmode=str(postgres_raw.get("sslmode") or "").strip(),
    )
    ...
    tables = _stringify_mapping(data.get("tables") or {}, "tables")
    ...
    for query_id, query_value in queries_raw.items():
        ...
        query_definitions[query_id_text] = QueryDefinition(
            ...
            file=resolve_query_path(base_dir, queries_dir, query_file_raw),
            output=str(query_value.get("output") or "").strip(),
            engine=str(query_value.get("engine") or "postgres").strip().lower(),
