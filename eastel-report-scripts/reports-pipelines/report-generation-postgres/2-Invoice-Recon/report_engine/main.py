...
if __package__ in (None, ""):
    ...
    from report_engine.query_executor import ExecutionContext, QueryExecutionResult, create_execution_context, execute_query
    ...
    from .query_executor import ExecutionContext, QueryExecutionResult, create_execution_context, execute_query
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the PostgreSQL invoice reconciliation Excel report.")
    parser.add_argument(
        "--config",
...
def run_report(config: AppConfig) -> tuple[Path, Path]:
    ...
    LOGGER.info("PostgreSQL database configured: %s", config.postgres.database or "(dsn/env)")
    LOGGER.info("Mongo database configured for SMSC/reference lookups: %s", config.mongo.database)
    LOGGER.info(
    ...
    execution_context = create_execution_context(config)
    try:
