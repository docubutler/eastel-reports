...
def validate_config(config: AppConfig) -> None:
    ...
    for query_id, query_definition in config.queries.items():
        ...
        if query_definition.engine not in {"postgres", "mongo_a2p_sms"}:
            raise ValueError(
                f"Unsupported engine '{query_definition.engine}' for query {query_id}. "
                "Expected 'postgres' or 'mongo_a2p_sms'."
            )
