...
SELECT
    ROUND(SUM(CASE WHEN t.rating_group = 'ONNET' THEN COALESCE(t.act_update_used_volume, 0) / 60.0 ELSE 0 END), 2) AS onnet_mou,
    ROUND(SUM(CASE WHEN t.rating_group = 'OFFNET' THEN COALESCE(t.act_update_used_volume, 0) / 60.0 ELSE 0 END), 2) AS offnet_mou
