# Temporary June 2026 1A Monthly Summary

This temporary folder is a one-off copy of `1A-Monthly-Summary` for the June 2026 run after usage-log data was split between:

- `iot_portal_tb_usage_log_old202606`
- `iot_portal_tb_usage_log`

Usage-log queries read the full June window from both tables with `UNION ALL` and filter by `variables.account_id` in `config.yml`. SMSC CDR queries intentionally do not use `account_id` because those rows do not have that field.

# Report Generation Scripts

This folder now contains two Python scripts:

- `split_queries.py`
  Splits [queries.sql](D:/DB_repos/eastel-reports/eastel-report-scripts/report-generation/report-generation-postgres/queries.sql) into numbered files such as `1.sql`, `2.sql`, `3.sql` inside the folder configured in `config.yml`.
- `generate_report.py`
  Reads those numbered SQL files, replaces SQL variables from `config.yml`, executes them on PostgreSQL, and writes a new report CSV from [report-template.csv](D:/DB_repos/eastel-reports/eastel-report-scripts/report-generation/report-generation-postgres/report-template.csv) without modifying the template.

## Files

- [config.yml](D:/DB_repos/eastel-reports/eastel-report-scripts/report-generation/report-generation-postgres/config.yml)
- [queries.sql](D:/DB_repos/eastel-reports/eastel-report-scripts/report-generation/report-generation-postgres/queries.sql)
- [report-template.csv](D:/DB_repos/eastel-reports/eastel-report-scripts/report-generation/report-generation-postgres/report-template.csv)
- [split_queries.py](D:/DB_repos/eastel-reports/eastel-report-scripts/report-generation/report-generation-postgres/split_queries.py)
- [generate_report.py](D:/DB_repos/eastel-reports/eastel-report-scripts/report-generation/report-generation-postgres/generate_report.py)

## Config

`config.yml` has four sections relevant to this flow:

```yaml
postgres:
  dsn: ""
  host: "localhost"
  port: 5432
  database: "anchor_iot"
  user: "postgres"
  password: "postgres"
  sslmode: "disable"

report_generation:
  source_queries_file: "queries.sql"
  split_queries_dir: "generated-queries"
  template_csv: "report-template.csv"
  output_csv: "report-output.csv"
  query_column_name: "Query"
  default_table: "iot_portal_tb_request_log"

tables:
  request_log_table: "iot_portal_tb_request_log"

variables:
  start_date: "2026-05-11"
  end_date: "2026-05-11"
```

### What each key does

- `report_generation.source_queries_file`
  Source SQL file containing all numbered queries.
- `report_generation.split_queries_dir`
  Output folder for `1.sql`, `2.sql`, `3.sql`, and so on.
- `report_generation.template_csv`
  Input template CSV. This file is never modified.
- `report_generation.output_csv`
  Final CSV written by the report runner.
- `report_generation.query_column_name`
  Column where the executed SQL text is written in the final report.
- `report_generation.default_table`
  Fallback table name if a query uses `{{default_table}}`.
- `tables`
  Named table variables that can be referenced inside the SQL source, such as `{{request_log_table}}`.
- `variables`
  Global SQL variables injected into the split queries at runtime.

## Step 1: Split the source SQL

Run:

```powershell
python split_queries.py
```

What it does:

- Reads `queries.sql`
- Detects query sections, preferably with `-- QUERY: <number> | <title>`
- Creates numbered files in `report_generation.split_queries_dir`
- Replaces hardcoded `t.req_time` date literals with:
  - `{{start_date}}`
  - `{{end_date_exclusive}}`

Example generated SQL:

```sql
SELECT
    t.rat_type,
    COUNT(*) AS total_transaction
FROM iot_portal_tb_request_log t
WHERE
    t.req_time >= '{{start_date}}'
    AND t.req_time < '{{end_date_exclusive}}';
```

## Step 2: Generate the report

Run:

```powershell
python generate_report.py
```

What it does:

- Reads database settings from `postgres`
- Reads SQL variables from `variables` and named table values from `tables`
- Reads the template CSV first and identifies which query ids are actually referenced
- Reads only the referenced `*.sql` files from `report_generation.split_queries_dir`
- Replaces placeholders like `{{start_date}}`, `{{end_date_exclusive}}`, and `{{request_log_table}}`
- Executes only the referenced queries against PostgreSQL
- Reads the template CSV row by row
- Replaces template placeholders such as `%1.total_transaction%`
- Writes a new file to `report_generation.output_csv`
- Writes the final executed SQL into the `Query` column
- Logs script start/end, per-query execution progress, row counts, and durations

## Template placeholder format

The runner understands placeholders in this form:

```text
%query_number.column_name%
```

Examples:

- `%1.total_transaction%`
- `%1.mou_minutes%`
- `%5.mou_MBs%`

Notes:

- Column lookup is case-insensitive, so `%5.mou_MBs%` matches SQL alias `mou_mbs`.
- If a query returns multiple rows, numeric columns are summed before substitution.
  This is useful for grouped queries like international voice or SMS summaries.
- If a placeholder refers to a missing value, the output cell is left blank.
- Only query ids referenced in the CSV template are executed.

## Expected flow

1. Update `variables.start_date` and `variables.end_date` in `config.yml`
2. Set `tables.request_log_table` if the source table name changes
3. Run `python split_queries.py`
4. Run `python generate_report.py`
5. Open the generated CSV from `report_generation.output_csv`

## Dependencies

These scripts require:

- `python`
- `PyYAML`
- `psycopg`

If they are not installed already:

```powershell
pip install PyYAML psycopg[binary]
```
